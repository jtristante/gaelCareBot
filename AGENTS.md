...
└── requirements.txt      # No pyproject.toml — deps go here
```
...
| Task | Location | Notes |
|------|----------|-------|
| Add a new command | `src/handlers/<name>.py` | Follow ConversationHandler or CommandHandler pattern |
| Register a handler | `src/bot.py:53-61` | Import symbol → `application.add_handler(...)` |
| Add a UI string | `src/messages.py` | All user-facing text lives here, `.format()` style |
| Add a config var | `src/config.py` | Extend `Config` dataclass + `.env.example` |
| DB schema changes | `src/db.py:_create_tables()` + `_migrate_schema()` | Add migration block, soft-fail if column exists |
| Change auth logic | `src/auth.py` | `_authorized_ids` module-level set |
| Group notifications | `src/group_notifier.py` | `send_daily_summary()` — called after mutations |
| Change deployment | `Dockerfile`, `docker-compose.yml` | See `devops-notes.md` (gitignored) |
| Run tests | `pytest` | `asyncio_mode=auto`, `pythonpath=src .` |

## CODE MAP

| Symbol | Type | Location | Role |
|--------|------|----------|------|
| `main()` | function | `src/bot.py:29` | Bootstrap: config → db → auth → handlers → poll |
| `Config` | dataclass | `src/config.py:16` | Frozen config from env vars |
| `load_config()` | function | `src/config.py:28` | Reads env → Config |
| `MilkDatabase` | class | `src/db.py:45` | SQLite wrapper: CRUD, FIFO consume, migrations |
| `init_auth()` | function | `src/auth.py:22` | Sets `_authorized_ids` from Config |
| `authorized_only` | decorator | `src/auth.py:42` | Gates every handler — checks `_authorized_ids` |
| `init_notifier()` | function | `src/group_notifier.py:29` | Stores Config for daily summaries |
| `send_daily_summary()` | function | `src/group_notifier.py:82` | Push summary to group chat (async) |
...
**Messages**: All user-facing strings in `src/messages.py` as UPPER_CASE constants. `.format(**kwargs)` with named placeholders. Never hardcode strings in handlers.
...
- **Do NOT add `pyproject.toml`/`setup.py` yet** — project uses flat `requirements.txt` + `python -m src.bot`.
...
```bash
# Development
python -m src.bot              # Run bot directly
